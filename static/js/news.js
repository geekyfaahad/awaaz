...
class NewsManager {
    ...
    createNewsCard(news) {
        const imageHTML = news.image && news.image !== 'No image available' 
            ? `<img src="${news.image}" class="news-image" alt="${news.headline || 'News Image'}" loading="lazy" onerror="this.style.display='none'">`
            : '';
        
        return `
            <div class="news-card" data-news-id="${this.generateNewsId(news)}">
                ${imageHTML}
                <div class="news-content">
                    <h5 class="news-title">${this.escapeHtml(news.headline || 'No Title')}</h5>
                    <p class="news-summary">${this.escapeHtml(news.summary || 'No Summary Available')}</p>
                    <div class="news-meta">
                        <span class="news-time">
                            <i class="far fa-clock"></i>
                            ${this.escapeHtml(news.time || 'Unknown Time')}
                        </span>
                        <a href="${this.escapeHtml(news.link || '#')}" target="_blank" rel="noopener noreferrer" class="btn-read-more">
